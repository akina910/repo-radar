...
The check script exits non-zero when the collector reports `degraded`, a runtime binding/secret is missing, D1/KV reads fail, or the latest snapshot is more than 2 days old. Stale history usually means the first manual sync has not run or the Worker cron needs attention.
